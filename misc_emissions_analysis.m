...
%                 end
        ...
        function make_oh_data_file(varargin)
            % Create the data file containing OH concentrations solved
            % using steady state models, simple HNO3 assumptions, or
        ...
        function delta = do_uncertainty(time_per, phox_in, alpha_in, tau, locs_to_calc, hcho_umtwrfs)
            % Compute uncertainty in OH concentrations
        ...
        function fit_wrf_oh_with_ss(loc_names)
            % Try to match WRF [OH] with the steady state models
            ss_locs = misc_emissions_analysis.read_locs_file();
        ...
        function [figs, oh_values] = plot_oh_conc_by_year(varargin)
            % Plot [OH] found by multiple methods
            E = JLLErrors;
            p = advInputParser;
...
%             locs = struct('Location', {'Alpha', 'Beta'});
        ...
        function figs = plot_if_oh_sig_different(varargin)
            % Plot of [OH] are significantly different
            E = JLLErrors;
            p = advInputParser;
            p.addParameter('loc_inds', nan);
            p.addParameter('oh_types', 'all'); % match the fns (invert, hno3, wrf, ratio). cell array if oh_type plot, string if cities plot
            ...
            for i_loc = 1:n_locs
                ...
                for i_fn = 1:n_fn
                    ...
                    ax = subplot(ny,nx,i_fn);
                    misc_emissions_analysis.plot_diff_sig_grid(years, this_oh, this_oh_err, this_ndofs, ax);
                    if i_fn == 1
        ...
        function figs = plot_ss_tau_vs_fit_tau(varargin)
            % Plot lifetime from the steady state models against the fitted
            % lifetime.
            p = advInputParser;
        ...
        function figs = plot_fit_ss_properties(varargin)
            % Plot [OH], VOCR, ALPHA, PHOX from the steady state models
            E = JLLErrors;
            p = advInputParser;
        ...
        function [figs, x_vals, y, yerr] = plot_avg_lifetime_change(varargin)
            ...
            %       * 'Inverted [OH]': plot [OH] from a steady state model.
            %       * 'HNO3 [OH]': plot [OH] assuming lifetime solely due
            %       to NO2 + OH
            %       * 'WRF [OH]': plot [OH] from the WRF model
            %
            ...
            for i_yr = 1:n_years
                ...
                for i_loc = 1:numel(week_locs)
                    ...
                    elseif strcmpi(plot_quantity, pqopts.ohss)
                        ...
                        warning('OH uncertainty not implemented')
                        this_week_err = NaN;
                        this_weekend_err = NaN;
            ...
            function [locs_wkday, locs_wkend] = load_oh(this_year_window)
